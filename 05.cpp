...
namespace {
    ...
    std::vector<ll> seeds = {4088478806, 114805397, 289354458, 164506173, 1415635989, 166087295, 1652880954, 340945548,
                             3561206012, 483360452, 35205517, 252097746, 1117825174, 279314434, 3227452369, 145640027,
                             2160384960, 149488635, 2637152665, 236791935};

    std::vector<std::string> seed_to_soil{
    ...
    std::vector<std::string> soil_to_fertilizer{
    ...
    std::vector<std::string> fertilizer_to_water{
    ...
    std::vector<std::string> water_to_light{
    ...
    std::vector<std::string> light_to_temperature{
            {"72585995 0 6987206"},
    ...
    std::vector<std::string> temperature_to_humidity{
    ...
    std::vector<std::string> humidity_to_location{
    ...
    std::vector<ll> split(const std::string &s, char delim) {
        ...
        while (getline(ss, item, delim)) {
            char *end = nullptr;
            result.push_back(strtoll(item.c_str(), &end, 10));
            assert(end == item.c_str() + item.size());
        }
    ...
    std::vector<std::tuple<ll, ll, ll>> split(const std::vector<std::string> &v) {
        std::vector<std::tuple<ll, ll, ll>> result;
        for (auto &line: v) {
            const std::vector<ll> &t = split(line, ' ');
            auto from_dest = t.at(0);
            auto from_src = t.at(1);
            auto len = t.at(2);
            result.emplace_back(from_dest, from_src, len);
        }
        return result;
    }

    ll lookup_dest(const std::vector<std::tuple<ll, ll, ll>> &m, ll src) {
        for (auto &map: m) {
            auto [from_dest, from_src, len] = map;
            ll to_dest = from_dest + len;
            ll to_src = from_src + len;

            if (src >= from_src && src < to_src) {
                ll offset = src - from_src;
                return from_dest + offset;
            }
        }
        return src;
    }
...
int main05() {
    ...
    auto seed_to_soil_m = split(seed_to_soil);
    auto soil_to_fertilizer_m = split(soil_to_fertilizer);
    auto fertilizer_to_water_m = split(fertilizer_to_water);
    auto water_to_light_m = split(water_to_light);
    auto light_to_temperature_m = split(light_to_temperature);
    auto temperature_to_humidity_m = split(temperature_to_humidity);
    auto humidity_to_location_m = split(humidity_to_location);
...
// humidity_to_location
    ...
    ll min_location = INT64_MAX;
    for (ll seed: seeds) {
        auto location = lookup_dest(humidity_to_location_m, lookup_dest(temperature_to_humidity_m,
                                                                        lookup_dest(light_to_temperature_m,
                                                                                    lookup_dest(water_to_light_m,
                                                                                                lookup_dest(
                                                                                                        fertilizer_to_water_m,
                                                                                                        lookup_dest(
                                                                                                                soil_to_fertilizer_m,
                                                                                                                lookup_dest(
                                                                                                                        seed_to_soil_m,
                                                                                                                        seed)))))));
        min_location = std::min<ll>(min_location, location);
    }
    ASSERT_EQ(min_location, 382895070);

    min_location = INT64_MAX;
    for (ll i = 0; i < seeds.size() - 1; i += 2) {
        ll from_seed = seeds.at(i);
        ll to_seed = from_seed + seeds.at(i + 1);

        for (ll seed = from_seed; seed < to_seed; seed++) {
            auto location = lookup_dest(humidity_to_location_m, lookup_dest(temperature_to_humidity_m,
                                                                            lookup_dest(light_to_temperature_m,
                                                                                        lookup_dest(water_to_light_m,
                                                                                                    lookup_dest(
    ...
    ASSERT_EQ(min_location, 17729182);
