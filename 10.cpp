...
namespace {
    ...
    decltype(input_real) input_test3{
            "7-F7-",
            ".FJ|7",
            "SJLL7",
            "|F--J",
    ...
    decltype(input_real) input_test4{
            "...........",
            ".S-------7.",
            ".|F-----7|.",
            ".||.....||.",
    ...
    decltype(input_real) input_test5{
            "FF7FSF7F7F7F7F7F---7",
            "L|LJ||||||||||||F--J",
            "FL-7LJLJ||||||LJL-77",
            "F--JF--7||LJLJ7F7FJ-",
...
//    auto &input = input_test5;
//    char S_REPLACEMENT = 'f'; // s replacement is F
    char S_REPLACEMENT = 'm'; // s replacement is -
//    char S_REPLACEMENT = 's'; // s replacement is 7


    std::tuple<ll, ll> find_S() {
    ...
    using CoordinateDiff = std::tuple<ll, ll>;
    constexpr ll SameX = 0;
    constexpr ll SameY = 0;
    constexpr ll FromAbove = -1;
    constexpr ll FromBelow = 1;
    constexpr ll FromLeft = -1;
    constexpr ll FromRight = 1;

    // This map shows where will the given pipe lead us
    // if we came from a coordinate with the given X and Y offset.
    //
    // During the implementation of traverse() I realised that it would be
    // sufficient just to know the keys of this map. However checking
    // the target of the pipe reduced the number of recursion (probably).
    const std::map<std::tuple<char, CoordinateDiff>, CoordinateDiff> Connections{
            {{'|', {SameX,     FromAbove}}, {SameX, +2}},
            {{'|', {SameX,     FromBelow}}, {SameX, -2}},

            {{'-', {FromLeft,  SameY}},     {+2,    SameY}},
            {{'-', {FromRight, SameY}},     {-2,    SameY}},

            {{'L', {SameX,     FromAbove}}, {+1,    +1}},
            {{'L', {FromRight, SameY}},     {-1,    -1}},

            {{'J', {SameX,     FromAbove}}, {-1,    +1}},
            {{'J', {FromLeft,  SameY}},     {+1,    -1}},

            {{'7', {FromLeft,  SameY}},     {+1,    +1}},
            {{'7', {SameX,     FromBelow}}, {-1,    -1}},

            {{'F', {SameX,     FromRight}}, {+1,    -1}},
            {{'F', {FromRight, SameY}},     {-1,    +1}},
    };


    struct Path {
        std::tuple<ll, ll> start;
        std::vector<std::tuple<ll, ll>> path_coord;

        bool is_visited(ll x, ll y) const {
            return std::find(path_coord.begin(), path_coord.end(), std::tuple<ll, ll>{x, y}) != path_coord.end();
        }
    };

    std::vector<Path> all_loops;

    bool is_valid_coord(ll x, ll y) {
        if (y >= input.size()) {
            return false;
        }
        if (x >= input.at(0).size()) {
            return false;
        }
        return true;
    }

    const std::set<char> valid_pipes = {'|', '-', 'L', 'J', '7', 'F'};

    void traverse(Path &path) {
        auto [current_x, current_y] = path.path_coord.back();

        std::vector<Path> new_paths;

        for (auto [dx, dy]: std::vector<std::tuple<ll, ll>>{{-1, 0},
                                                            {1,  0},
                                                            {0,  -1},
                                                            {0,  1}}) {
            ll pipe_x = current_x + dx;
            ll pipe_y = current_y + dy;
            if (path.is_visited(pipe_x, pipe_y)) {
                continue;
            }
            if (!is_valid_coord(pipe_x, pipe_y)) {
                continue;
            }
            char pipe = input.at(pipe_y).at(pipe_x);
            auto it = Connections.find({pipe, {dx * -1, dy * -1}});
            if (it != Connections.end()) {
                auto [connected_x_diff, connected_y_diff] = it->second;
                auto connected_x = current_x + connected_x_diff;
                auto connected_y = current_y + connected_y_diff;

                if (is_valid_coord(connected_x, connected_y)) {
                    if (input.at(connected_y).at(connected_x) == 'S') {
                        path.path_coord.push_back({pipe_x, pipe_y});
                        all_loops.push_back(path);
                    } else if (input.at(connected_y).at(connected_x) == '.') {
                        continue;
                    } else if (!path.is_visited(connected_x, connected_y)) {
                        assert(valid_pipes.contains(pipe));
                        Path new_path = path;
                        new_path.path_coord.push_back({pipe_x, pipe_y});
...
int main10() {
    ...
    {
        Path p;
        p.path_coord.push_back(start);
        traverse(p);

        // traverse finds two loops, which are just clock and counterclockwise wise equivalents
        ASSERT_EQ(all_loops.size(), 2);
        ASSERT_EQ(all_loops.at(0).path_coord.size(), all_loops.at(1).path_coord.size());
        myprint(all_loops.at(0).path_coord.size());
        myprint(all_loops.at(0).path_coord.size() / 2);
    }

    const Path &loop = all_loops.at(0);
    std::vector<std::string> zoom(input.size() * 3);

    ll flood_start_x;
    ll flood_start_y;

    // The idea is to count tiles by means of flooding.
    // The problem of this approach is that some walls are located too close to each other.
    // There is no continuous space.
    // In order to achieve the flooding we do a 3x zoom-in
    //   1. Replace every 1x1 pipe by 3x3 pipe
    //       "|" becomes " | "   "-"  becomes "   "   "L" becomes  " | "
    //                   " | "                "---"                " L-"
    //                   " | "                "   "                "   "
    //
    //       "F" becomes "   "   "J"  becomes " | "   "7" becomes  "   "
    //                   " F-"                "-J "                "-7 "
    //                   " | "                "   "                " |
    //   2. Replace every previous non-path artifact with ".  "
    //                                                    "   "
    //                                                    "   "
    //   3. Start flooding close to the 'S' and count the number of '.'s
    //
    // The downside of this approach:
    // I didn't come up with a heuristic to choose the starting point for a flooding.
    // I printed out the zoomed path and selected the point manually.

    for (ll y = 0; y < input.size(); y++) {
        ll yy = y * 3;
        zoom.at(yy).resize(input.at(y).size() * 3, ' ');
        zoom.at(yy + 1).resize(input.at(y).size() * 3, ' ');
        zoom.at(yy + 2).resize(input.at(y).size() * 3, ' ');
        for (ll x = 0; x < input.at(y).size(); x++) {
            ll xx = x * 3;
            if (loop.is_visited(x, y)) {
                char pipe = input.at(y).at(x);
                if (pipe == 'S') {
                    pipe = S_REPLACEMENT;
                }
                if (pipe == '|') {
                    zoom.at(yy).at(xx + 1) = '|';
                    zoom.at(yy + 1).at(xx + 1) = '|';
                    zoom.at(yy + 2).at(xx + 1) = '|';
                } else if (pipe == '-') {
                    zoom.at(yy + 1).at(xx) = '-';
                    zoom.at(yy + 1).at(xx + 1) = '-';
                    zoom.at(yy + 1).at(xx + 2) = '-';
                } else if (pipe == 'L') {
                    zoom.at(yy).at(xx + 1) = '|';
                    zoom.at(yy + 1).at(xx + 1) = 'L';
                    zoom.at(yy + 1).at(xx + 2) = '-';
                } else if (pipe == 'J') {
                    zoom.at(yy).at(xx + 1) = '|';
                    zoom.at(yy + 1).at(xx + 1) = 'J';
                    zoom.at(yy + 1).at(xx) = '-';
                } else if (pipe == '7') {
                    zoom.at(yy + 1).at(xx) = '-';
                    zoom.at(yy + 1).at(xx + 1) = '7';
                    zoom.at(yy + 2).at(xx + 1) = '|';
                } else if (pipe == 'F') {
                    zoom.at(yy + 1).at(xx + 1) = 'F';
                    zoom.at(yy + 1).at(xx + 2) = '-';
                    zoom.at(yy + 2).at(xx + 1) = '|';
                } else if (pipe == 'f') {
                    zoom.at(yy + 1).at(xx + 1) = 'S';
                    zoom.at(yy + 1).at(xx + 2) = '-';
                    zoom.at(yy + 2).at(xx + 1) = '|';

                    // ATTENTION: input specific
                    flood_start_x = xx + 2;
                    flood_start_y = yy + 2;
                } else if (pipe == 'm') {
                    zoom.at(yy + 1).at(xx) = '-';
    ...
    std::vector<std::tuple<ll, ll>> to_visit;
    to_visit.push_back({flood_start_x, flood_start_y});
    int64_t number_of_tiles = 0;
    while (!to_visit.empty()) {
        auto [current_x, current_y] = to_visit.back();
        to_visit.pop_back();
        visited.insert({current_x, current_y});
        char current_char = zoom.at(current_y).at(current_x);
        if (current_char == '.') {
            zoom.at(current_y).at(current_x) = 'I';
            number_of_tiles++;
        } else {
            zoom.at(current_y).at(current_x) = 'x';
        }
        for (auto [dx, dy]: std::vector<std::tuple<ll, ll>>{{-1, 0},
                                                            {1,  0},
                                                            {0,  -1},
                                                            {0,  1}}) {
            ll neighbor_x = current_x + dx;
            ll neighbor_y = current_y + dy;

            if (visited.contains({neighbor_x, neighbor_y})) {
                continue;
            }
            if (neighbor_y < 0 || neighbor_y >= zoom.size()) {
                continue;
            }
            if (neighbor_x < 0 || neighbor_x >= zoom.at(neighbor_y).size()) {
                continue;
            }
            char symbol = zoom.at(neighbor_y).at(neighbor_x);
            if (valid_pipes.contains(symbol) || symbol == 'S') {
                continue;
            }
    ...
    myprint(number_of_tiles);
    ASSERT_EQ(number_of_tiles, 353);
    return 0;
}
