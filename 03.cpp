...
//    auto &input = input_test;
    ...
    bool is_symbol(ll line, ll pos) {
        if (line < 0 || line >= input.size()) {
            return false;
        }
        if (pos < 0 || pos >= input.at(line).size()) {
            return false;
        }
        return is_symbol(input.at(line).at(pos));
    }

    bool is_gear(ll line, ll pos) {
        if (line < 0 || line >= input.size()) {
            return false;
        }
        if (pos < 0 || pos >= input.at(line).size()) {
...
int main03() {
    ...
    for (ll line = 0; line < input.size(); line++) {
        auto &l = input.at(line);
        for (const char *it = l.c_str(); it < l.c_str() + l.size();) {
            if (isdigit(*it)) {
                char *end_ptr = nullptr;
                auto value = strtol(it, &end_ptr, 10);
                auto value_begin = it - l.c_str();
                auto value_end = end_ptr - l.c_str() - 1;

                for (ll x = value_begin - 1; x <= value_end + 1; ++x) {
                    for (ll y = line - 1; y <= line + 1; ++y) {
                        if (is_symbol(y, x)) {
                            result0 += value;
                            goto found_adj;
                        }
    ...
    using GearXY = std::tuple<ll, ll>;
    using NumberXYValue = std::tuple<ll, ll, ll>;
    std::map<GearXY, std::set<NumberXYValue>> map;
    for (ll line = 0; line < input.size(); line++) {
        auto &l = input.at(line);
        for (const char *it = l.c_str(); it < l.c_str() + l.size();) {
            if (isdigit(*it)) {
                ...
                for (ll x = value_begin - 1; x <= value_end + 1; ++x) {
                    for (ll y = line - 1; y <= line + 1; ++y) {
                        if (is_gear(y, x)) {
                            map[{x, y}].insert({value_begin, line, value});
                        }
                    }
                }
                it = end_ptr;
    ...
    for (auto &[gear, numbers]: map) {
        if (numbers.size() == 2) {
            auto [x0, y0, value0] = *numbers.begin();
            auto [x1, y1, value1] = *numbers.rbegin();
            result1 += (value0 * value1);
        }
    }

    ASSERT_EQ(result0, 527364);
    ASSERT_EQ(result1, 79026871);
    return 0;
}
