...
//    auto &input = input_test;
    ...
    std::map<ll, ll> replace0(const std::string &value) {
        std::map<ll, ll> result;
        for (ll i = 0; i < value.size(); ++i) {
            if (isdigit(value.at(i))) {
                result.insert({i, value.at(i) - '0'});
                continue;
            }
        }
        return result;
    ...
    std::map<ll, ll> replace(const std::string &value, bool decode_words) {
        static const std::string words[9] = {"one",
                                             "two",
                                             "three",
                                             "four",
                                             "five",
                                             ...
                                             "nine"};
        std::map<ll, ll> result;
        for (ll char_i = 0; char_i < value.size(); ++char_i) {
            if (isdigit(value.at(char_i))) {
                result.insert({char_i, value.at(char_i) - '0'});
                continue;
            }
            if (decode_words) {
                ll word_i = 1;
                for (auto &word: words) {
                    if (strncmp(value.c_str() + char_i, word.c_str(), word.size()) == 0) {
                        result.insert({char_i, word_i});
                    }
                    ++word_i;
                }
            }
...
int main01() {
    ...
    for (const auto &i: input) {
        auto map = replace(i, false);
        result0 += (map.begin()->second * 10 + map.rbegin()->second);
    }
    ASSERT_EQ(result0, 54304);

    int64_t result1 = 0;
    for (const auto &i: input) {
        auto map = replace(i, true);
        result1 += (map.begin()->second * 10 + map.rbegin()->second);
    }
    ASSERT_EQ(result1, 54418);
    return 0;
}
